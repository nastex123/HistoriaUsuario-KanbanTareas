...
export function dashboardView() {
  return `
    ...
    <div id="editTaskModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 hidden">
      <div class="bg-surface rounded-xl p-6 w-full max-w-md">
        <h3 class="text-headline-md mb-4">Edit Task</h3>
...
export async function initDashboard() {
  ...
  await loadTasks(users, isAdmin);

  if (isAdmin) {
    setupCreateModal(users);
    setupEditModal(users);
  }
}

// Carga y pinta tareas
async function loadTasks(users, isAdmin = false) {
  try {
    ...
    tasks.forEach(task => {
      ...
      // Determinar texto de estado según el estado de la tarea
      let statusText = '';
      let statusIcon = '';
      switch (status) {
      ...
      card.innerHTML = `
        <div class="flex items-start justify-between mb-xs">
          ...
          <div class="flex items-center gap-1">
            ...
            ${isAdmin ? `<span class="edit-task-btn material-symbols-outlined text-outline hover:text-primary" data-task-id="${task.id}">edit</span>` : ''}
          </div>
        </div>
        ...
        <div class="mt-md flex items-center justify-between">
          <div class="avatar-group">
            ...
            <span class="font-label-sm text-label-sm text-outline">${assigneeName}</span>
          </div>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await loadTasks(updatedUsers, isAdmin);
...
async function openEditModal(taskId, users) {
  try {
